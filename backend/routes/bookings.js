...
const createBooking = async (req, res) => {
  const clientId = req.body.clientid || req.body.clientId || req.body.client_id;
  const providerId = req.body.providerid || req.body.providerId || req.body.provider_id;
  ...
  if (!clientId || !providerId || !bookingDate || !bookingTime) {
  ...
  try {
    const [rows] = await db.query(
      `SELECT ub.id, ub.clientId, ub.providerId, ub.bookingDate, ub.bookingTime, ub.status,
       ...
       LEFT JOIN provider_profiles pp ON pp.user_id = ub.providerId
    ...
    try {
      const [rows] = await db.query(
        `SELECT b.id, b.client_id AS clientId, b.provider_id AS providerId,
...
const updateBookingStatus = async (req, res) => {
  ...
  try {
    ...
    try {
      const [result] = await db.query('UPDATE Bookings SET status = ? WHERE id = ?', [
        normalizeStatusForLegacyTable(status),
        bookingId
      ]);
      ...
      const [updatedRows] = await db.query('SELECT provider_id FROM Bookings WHERE id = ?', [bookingId]);
